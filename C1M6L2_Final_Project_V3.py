...
def calculate_frequencies(file_contents):
    ...
    for word in txt:
        freq=" "
        for cloud in word:
            if cloud not in punctuations:
                freq=freq+cloud
        if freq not in uninteresting_words:
            if freq not in iteration_dic.keys():
                iteration_dic[freq]=1
            else:
                iteration_dic[freq]+=1
    
    #wordcloud
    cloud = wordcloud.WordCloud()
    cloud.generate_from_frequencies(iteration_dic)
